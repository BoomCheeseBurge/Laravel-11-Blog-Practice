<x-layouts.dashboard-layout :page="$page" :title="$title">
    ...
    <div class="w-full p-10 bg-white rounded-md shadow-lg shadow-slate-300 dark:bg-slate-600 dark:shadow-slate-700">
        <form action="{{ route('users.update', ['user' => $user->username]) }}" id="registrationForm" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <section class="grid gap-y-5 my-8 md:grid-cols-2 md:gap-x-8">
                ...
                <div class="relative">
                    ...
                    @else
                    is-valid
                    @enderror" placeholder="Enter your fullname" value="{{ old('fullname') ?? $user->fullname }}" autofocus autocomplete="off" >
                ...
                {{-- ============================================================ Email Input ============================================================ --}}
                <div class="relative">
                    <label for="email" class="text-graydark relative block mb-2 text-base font-medium dark:text-white md:text-md">Email Address</label>
                    ...
                    @else
                    is-valid
                    @enderror" placeholder="name@domain.com" value="{{ old('email') ?? $user->email }}" >
                    <svg class="w-6 h-6 text-red-600 dark:text-rose-500
                    @error('email')
                    absolute top-11 right-0 mr-3
                    @else
            ...
            <section class="grid gap-y-8 my-10 md:grid-cols-2 md:gap-x-16">
                ...
                <div class="relative">
                    ...
                    @else
                    is-valid
                    @enderror" placeholder="Enter your username" value="{{ old('username') ?? $user->username }}" >
                    <svg class="w-6 h-6 text-red-600 dark:text-rose-500
                    @error('username')
                    absolute top-11 right-0 mr-3
                    @else
                ...
                <div>
                    ...
                    @enderror
                    <div class="mt-1">
                        <textarea id="about" name="about" rows="6" class="bg-gray w-full block rounded-md border-2 border-slate-200 shadow-sm dark:placeholder-gray-3 dark:text-white dark:bg-slate-700 dark:border-slate-600 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ old('about') ?? $user->about }}</textarea>
                    </div>
            ...
            <section>
                <div class="w-full mt-5 dark:bg-slate-600">
                    <div class="mx-auto w-full">
                        ...
                        @else
                        <img src="{{ asset('IMG/default/default_background.jpeg') }}" alt="Default Profile Picture" id="cover_preview"
                            class="max-h-50 w-full object-fill rounded-md lg:max-h-75" />
                        @endif
                        ...
                        <div class="mb-25 mx-auto w-full relative flex justify-center">
                            ...
                            <img src="{{ Storage::disk('profile')->url($user->profile_pic) }}" alt="{{ $user->username }} Profile Picture" id="profile_preview"
            ...
            <div class="flex flex-col items-center">
                <button type="submit" class="bg-primary-600 w-full px-5 py-2.5 mt-5 text-lg font-medium text-white rounded-lg dark:bg-primary-600 dark:focus:ring-primary-800 dark:hover:bg-primary-700 focus:ring-primary-300 focus:outline-none focus:ring-4 hover:bg-primary-700 md:max-w-md">Save</button>
            </div>
        </form>
    </div>
