                                    ...
                                    <span wire:key="paginator-{{ $paginator->getPageName() }}-page{{ $page }}">
                                        @if ($page == $paginator->currentPage())
                                            <span aria-current="page">
                                                <span class="-ml-px relative inline-flex items-center px-4 py-2 text-sm font-bold leading-5 text-slate-900 border border-slate-300 cursor-default dark:text-white dark:bg-slate-800 dark:border-slate-600">{{ $page }}</span>
                                            </span>
                                        @else
                                            <button type="button" wire:click="gotoPage({{ $page }}, '{{ $paginator->getPageName() }}')" x-on:click="{{ $scrollIntoViewJsSnippet }}" class="-ml-px relative inline-flex items-center px-4 py-2 text-sm font-medium leading-5 text-slate-700 bg-white border border-slate-300 ring-blue-300 transition duration-150 ease-in-out active:text-slate-700 active:bg-slate-100 dark:text-slate-300 dark:bg-slate-800 dark:border-slate-600 dark:active:bg-slate-700 dark:focus:border-blue-800 dark:hover:text-slate-300 focus:z-10 focus:border-blue-300 focus:outline-none focus:ring hover:text-primary-custom hover:underline" aria-label="{{ __('Go to page :page', ['page' => $page]) }}">
                                                {{ $page }}
                                            </button>
                                        @endif
                                    </span>
