<thead class="bg-slate-700 dark:bg-indigo-100 max-md:[&_th]:hidden [&_th]:p-4">
    <tr>
        {{-- Header ID/No. START --}}
        @if ($attributes->has('id'))
        <th id="no" x-show="toggleColumn('No')" class="text-sm tracking-wider leading-4 text-center text-white border-b-2 border-slate-600 group dark:text-primary-600 dark:border-gray-300 md:text-base">
        ...
        @foreach (array_filter($headers, function (string $value) {
            ...
            <th id="{{ $header }}" x-show="toggleColumn('{{ $header }}')" class="border-x-[2px] border-x-primary-600/40 px-4 text-sm tracking-wider leading-4 text-left text-white border-b-2 border-b-slate-600 group dark:border-x-gray-400/40 dark:text-primary-600 dark:border-b-gray-300 md:text-base">
        ...
        <th id="action" x-show="toggleColumn('Action')" class="border-b-2 border-b-slate-600 dark:border-b-gray-300">
            <div class="hidden">
                {{-- Sort Button START --}}
